...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if st.button("POST /seed", use_container_width=True, key="seed_btn"):
            ...
            if s in (200, 201):
                st.markdown(
                    f'<div class="alert-ok">seeded — {r.get("users_created",0)}u · {r.get("urls_created",0)}url · {r.get("events_created",0)}evt</div>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(f'<div class="alert-err">{r.get("error","seed failed")}</div>', unsafe_allow_html=True)
